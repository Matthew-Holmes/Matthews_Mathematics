...
\newcommand{\piechart}[2]{%
  \begin{tikzpicture}
    ...
    \foreach \lbl/\ang/\col in {#2} {%
      ...
      \fill[\col] (0,0) -- (\startA:\r cm) arc(\startA:\endA:\r cm) -- cycle;%
      \draw[white, line width=2pt] (0,0) -- (\startA:\r cm);%
      \pgfmathsetmacro{\midA}{(\startA + \endA)/2}%
      \pgfmathsetmacro{\lR}{\r * 0.65}%
      \node[font=\small\bfseries, text=white, align=center] at (\midA:\lR cm) {\lbl};%
      \xdef\startA{\endA}%
    }%
    \draw[black, line width=1.8pt] (0,0) circle (\r cm);%
    \fill[black] (0,0) circle (3pt);%
    \draw[black, line width=1.2pt] (0,\r cm) -- (0,{\r cm+0.25cm});%
    \draw[gray!50, dashed, line width=0.7pt] (0,0) -- ({\r cm+0.2cm},0);%
  \end{tikzpicture}%
}
