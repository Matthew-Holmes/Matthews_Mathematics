...
\item Write the fraction represented by each bar.
...
\item (Extension) explain why fractions with the same numerator change size when the denominator changes.
...
\item Fully simplify: $\frac{18}{24}$
...
\item Write the improper fraction shown by the diagram.

\item Write the mixed number represented by the green bars in the diagram.

\item Mark $1\frac{1}{4}$ and $2\frac{3}{4}$ on the number line.

\item Convert: $\frac{13}{5}$ to a mixed number.
...
\item Write the fractions shown by the bars.
