...
\begin{scope}[shift={(#1,#2)}]
  ...
  \node at (-0.8,0.5) [fill=white, inner sep=1pt] {\tiny \heightstr};
    ...
    \coordinate (tickDir) at ($(dir)*0.1$);        % vector for tick length
    \draw ($(start)-(tickDir)$) -- ($(start)+(tickDir)$);
    \draw ($(end)-(tickDir)$) -- ($(end)+(tickDir)$);
    % label
...
\begin{tikzpicture}[scale=0.55] % Reduced scale
\def\R{3}
\def\r{1.5}
...
% Repositioned labels at midpoints of radii
\node at ($(0,0)!0.5!(\R,0)$) [yshift=-0.3cm] {$6$ cm};
\node at ($(0,0)!0.5!(0,\r)$) [xshift=0.1cm] {$3$ cm};
\end{tikzpicture}
...
\begin{tikzpicture}[scale=0.45] % Slightly reduced
...
{\color{red} Similar shapes: A, C, D}
