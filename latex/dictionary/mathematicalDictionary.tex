...
\dictentry{algebraic method}{a way of solving a problem by using letters to stand for the numbers you want to find.}
\dictentry{commutative}{describes an operation whose order does not affect the answer.}
\dictentry{unit fraction}{a fraction whose top number is one.}
\dictentry{algebraic}{using letters to stand for unknown numbers.}
\dictentry{bar chart}{a diagram using bars to show how many of each category there are}
\dictentry{counter-example}{an example that shows a general statement is false.}
\dictentry{decrease}{to make an amount smaller, usually by subtracting a percentage of the starting amount}
\dictentry{discount}{money taken off a normal price, often given as a percentage}
\dictentry{dot notation}{a way of writing a recurring decimal by putting a dot over the digit or digits that repeat.}
\dictentry{factor pair}{Two numbers that multiply together to make a chosen number.}
\dictentry{increase}{to make an amount larger, usually by adding a percentage of the starting amount}
\dictentry{index form}{A way of writing repeated multiplication using a small raised number, such as 2\textasciicircum{}3.}
\dictentry{interval}{the gap between two marks on a number line or scale.}
\dictentry{irrational}{a number whose decimal part never ends and never repeats, so it cannot be written as a fraction.}
\dictentry{original value}{the amount before a percentage increase or decrease is applied}
\dictentry{prime factor}{A factor that is also a prime.}
\dictentry{prime factor decomposition}{Writing a number as a product of its prime factors.}
\dictentry{rational}{a number that can be written as a fraction of two whole numbers.}
\dictentry{reduce}{to make an amount smaller, especially by removing a stated percentage from it}
\dictentry{terminating decimal}{a number whose digits after the point stop after a limited number of places.}
\dictentry{account}{a record with a bank for keeping money}
\dictentry{balance}{the amount of money in an account at a particular time}
\dictentry{cap}{an upper limit on how large an amount can be}
\dictentry{capped}{limited to a largest number or size.}
\dictentry{certain}{sure to happen; its probability is one}
\dictentry{compound interest}{interest calculated on the original amount plus interest already added}
\dictentry{cross-multiply}{multiply the top of each side by the bottom of the other in two equal fractions}
\dictentry{curve}{a line that bends instead of being straight.}
\dictentry{d.p}{short for decimal place}
\dictentry{deposit}{money you put into a savings account}
\dictentry{double}{to become twice as large.}
\dictentry{effective annual rate}{the true percentage change over one year when interest is added more than once}
\dictentry{equally likely}{having the same probability as each other}
\dictentry{even chance}{exactly as likely to happen as not; its probability is one half}
\dictentry{event}{something that may happen in a probability situation}
\dictentry{expect}{to work out the average number of times an outcome would happen over many goes}
\dictentry{fair}{where every possible outcome has the same probability}
\dictentry{fee}{money charged by a bank for a service}
\dictentry{frequency tree}{a diagram that splits a total into groups using two features}
\dictentry{graph}{a drawing that shows how one amount changes compared with another}
\dictentry{gross interest}{interest before any fees or charges are taken away}
\dictentry{impossible}{cannot happen; its probability is zero}
\dictentry{inclusive}{including the end number in a range}
\dictentry{less than or equal to}{a number is smaller than another number or the same as it}
\dictentry{likely}{has a probability more than one half but less than one}
\dictentry{locked in}{not able to take your money out for a set time.}
\dictentry{maximum}{the largest amount or number allowed}
\dictentry{midpoint}{the number exactly halfway between two other numbers}
\dictentry{minimum}{the smallest amount or number allowed}
\dictentry{multiplier}{a number that turns an original amount into a new amount for a percentage change}
\dictentry{never-ending decimal}{a number written with a decimal point whose digits go on without stopping}
\dictentry{outcome}{one of the possible results when something is done}
\dictentry{overlap}{the part where two groups share members}
\dictentry{plot}{to mark locations in the correct place on a graph}
\dictentry{positive number}{a number above zero, such as 5}
\dictentry{random}{chosen so that every outcome has the same probability}
\dictentry{repeating block}{a group of digits that repeats forever in a decimal}
\dictentry{repeating decimal}{a number written with a decimal point in which a digit or group of digits repeats forever}
\dictentry{round-half-up}{the rule that a number exactly halfway between two numbers goes to the larger number}
\dictentry{unlikely}{has a probability more than zero but less than one half}
\dictentry{venn diagram}{a diagram with circles that shows what different groups share}
\dictentry{withdraw}{to take money out of a bank.}
\dictentry{withdrawal}{money taken out of an account}
\dictentry{greater than}{larger than another amount}
\dictentry{less than}{smaller than another amount}
\dictentry{nearest}{the closest among a set of choices}
\dictentry{cube root}{a number that gives a specified value when multiplied by itself twice more}
\dictentry{descending order}{arranged from largest to smallest}
\dictentry{fifth root}{a number that gives a specified value when five copies are multiplied together}
\dictentry{fourth root}{a number that gives a specified value when four copies are multiplied together}
\dictentry{fractional index}{an index written as a fraction, such as one half or three quarters}
\dictentry{index law}{a rule for working with indices, such as adding them when multiplying}
\dictentry{place value chart}{a table that shows what each digit in a number is worth}
\dictentry{round down}{to replace a number with the smaller nearby number}
\dictentry{round up}{to replace a number with the larger nearby number}
\dictentry{step}{the distance from one mark to the next on a line}
\dictentry{double rounding}{changing a number to a simpler value twice, which can give the wrong answer.}
\dictentry{hundreds digit}{the digit three places from the right in a whole number.}
\dictentry{hundredths digit}{the digit in the second place after the decimal point.}
\dictentry{non-repeating}{not having a group of digits that comes round again forever.}
\dictentry{non-terminating}{going on forever without stopping.}
\dictentry{strict}{in an inequality, using only bigger or smaller, not allowing the same value.}
\dictentry{tens digit}{the digit two places from the right in a whole number.}
\dictentry{tenths digit}{the digit in the first place after the decimal point.}
\dictentry{trailing zero}{a zero at the end of a decimal number that does not change its value.}
\dictentry{units digit}{the digit in the rightmost place of a whole number.}
\dictentry{add}{to put numbers together to find their total}
\dictentry{algebra}{a branch of mathematics using letters to stand for unknown numbers}
\dictentry{algebraic fraction}{a fraction that contains letters as well as numbers}
\dictentry{algebraic variable}{a letter, such as a, x or m, that stands for a number that can change.}
\dictentry{bureau de change}{a place where you change money from one currency to another}
\dictentry{cancelling}{removing the same factor from the top and bottom of a fraction}
\dictentry{carry}{to move an extra ten to the next position on the left when a total is ten or more}
\dictentry{cell}{a small square or rectangle in a grid.}
\dictentry{column}{a line of cells going down a grid.}
\dictentry{column method}{a written way of doing multiplication by working with place values in up-and-down lines.}
\dictentry{commission}{a fee that is a share of the money being changed, not a fixed amount}
\dictentry{constant term}{a term in an expression that has no letter}
\dictentry{conversion graph}{a graph that shows how much of one currency you get for another}
\dictentry{currency}{the money used in a country}
\dictentry{decimal number}{a number that includes a point and shows a part less than one.}
\dictentry{diagonal}{a straight line that goes from one corner of a rectangle to the opposite corner}
\dictentry{difference of two squares}{an expression like one square minus another square}
\dictentry{exchange rate}{the amount of another country's money that one pound buys}
\dictentry{factor tree}{a diagram that splits a number into factors until only prime factors remain}
\dictentry{factorial}{the result of multiplying all whole numbers from a given number down to 1.}
\dictentry{flat fee}{a fixed charge taken off every exchange, whatever the amount}
\dictentry{grid}{a pattern of lines that makes boxes or squares for organising numbers}
\dictentry{hundreds}{the position in a number that counts groups of ten tens}
\dictentry{hypervolume}{the amount of space inside a four-dimensional shape, measured in fourth-power units.}
\dictentry{intersection}{the part where two circles in a Venn diagram overlap}
\dictentry{lattice}{a boxed diagram with diagonal lines used for multiplication}
\dictentry{leading zero}{a digit written at the front of a decimal to keep the other digits in their correct positions}
\dictentry{like surds}{two or more values with the same number under the root sign}
\dictentry{negative index}{a small raised number that is negative, telling you to take the reciprocal}
\dictentry{rectangular}{having four straight sides and four corners that are all the same.}
\dictentry{row}{a line of cells going across a grid.}
\dictentry{significant digits}{the figures in a number that show its true size, not noughts used only for position.}
\dictentry{solve}{to find the value that makes an equation true}
\dictentry{subtract}{to take one number away from another}
\dictentry{ten thousands}{the position in a number that counts groups of ten thousands}
\dictentry{tens}{the position in a number that counts groups of ten}
\dictentry{tesseract}{the four-dimensional shape made by moving a cube in a fourth direction.}
\dictentry{thousands}{the position in a number that counts groups of ten hundreds}
\dictentry{truncate}{to cut off extra digits instead of rounding the number}
\dictentry{units}{the position in a number that counts single ones}
\dictentry{y-intercept}{the value where a line crosses the vertical axis}
\dictentry{at random}{chosen so that every item has the same chance of being picked.}
\dictentry{biased}{not fair, so some results are more likely than others.}
\dictentry{branch}{one path in a frequency tree, leading to a group or subgroup.}
\dictentry{double inequality}{an inequality that gives both a lower and an upper limit for a value.}
\dictentry{filled circle}{a small solid circle on a number line showing that the marked value is included.}
\dictentry{open circle}{a small empty circle on a number line showing that the marked value is not included.}
\dictentry{ordinary number}{a number written in the usual way, not in standard form.}
\dictentry{satisfy}{make a statement true when a letter is replaced by that value.}
\dictentry{standard form}{a way of writing large or small numbers as a number between one and ten times a power of ten.}
\dictentry{strictly}{used to mean the end value is not included.}
